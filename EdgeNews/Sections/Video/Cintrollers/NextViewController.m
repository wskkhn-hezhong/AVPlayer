...
@interface NextViewController ()<UITableViewDelegate , UITableViewDataSource, VideoPlayCellDelegate, VideoPlayDelegate> {
    BOOL _ishidden;
    BOOL _isSmall;
}
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = self.head.title;
    [self backButton];
    [self tableView];
    [self netWork];
    self.page = 0;
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"heaee"];
...
//        playerView.fullScreenButton.frame = CGRectMake(HEIGHT - 40, 5, 30, 30);
        ...
        [[UIApplication sharedApplication].windows.firstObject addSubview:playerView];//添加进全屏
    }];
}
...
- (void)backButton {
    UIBarButtonItem *leftItem =[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"ba"] style:UIBarButtonItemStylePlain target:self action:@selector(clickMe)];
    self.navigationItem.leftBarButtonItem.tintColor = [UIColor colorWithRed:1.0 green:0.0 blue:0.0 alpha:1.0];
...
- (void)netWork {
    ...
    [manager GET:[NSString stringWithFormat:@"http://c.m.163.com/nc/video/list/%@/y/%ld-10.html",self.head.sid, self.page] parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
//             NSLog(@"JSON: %@", responseObject);
        
        for (NSDictionary *dic1 in responseObject[self.head.sid]) {
